...
var datePickerController = (function datePickerController() {
  ...
  function parseUILanguage() {
    ...
    return languageTag.search(/^([a-z]{2,3})-([a-z]{2})$/) != -1
      ? [languageTag.match(/^([a-z]{2,3})-([a-z]{2})$/)[1], languageTag]
      : [languageTag];
  }
  ...
  function parseJSON(str) {
    ...
    try {
      ...
      if (typeof JSON === "object" && JSON.parse) {
        ...
        /lang|buttontabindex|mousewheel|cellformat|titleformat|nodrag|describedby/.test(
          str.toLowerCase()
        )
      ) {
        var f = Function(
  ...
  function datePicker(options) {
    ...
    (this.cursorDate = options.cursorDate ? options.cursorDate : ""),
      (this.date = options.cursorDate
        ? new Date(
            +options.cursorDate.substr(0, 4),
            +options.cursorDate.substr(4, 2) - 1,
            +options.cursorDate.substr(6, 2)
          )
        : new Date());
    this.defaults = {};
    ...
    for (
      var i = 0, prop;
      (prop = ["callbacks", "formElements", "formatMasks"][i]);
      i++
    ) {
      this[prop] = {};
    ...
    this.createCbArgObj = function() {
      return this.dateSet
        ...
        : { id: this.id, date: null, dd: null, mm: null, yyyy: null };
    };
    ...
    this.reposition = function() {
      ...
      var osh = o.div.offsetHeight,
        ...
        fitsTop =
          parseInt(pos[1] - (osh + elem.offsetHeight + 2)) >
          parseInt(scrollTop);
      ...
      o.div.style.left =
        Number(
          parseInt(trueBody.clientWidth + scrollLeft) < parseInt(osw + pos[0])
            ? Math.abs(parseInt(trueBody.clientWidth + scrollLeft - osw))
            : pos[0]
        ) + "px";
      o.div.style.top =
        fitsBottom || !fitsTop
    ...
    this.addAccessibleDate = function() {
      ...
      if (td && !td.getElementsByTagName("span").length) {
        var ymd = td.className.match(/cd-([\d]{4})([\d]{2})([\d]{2})/),
          noS =
            td.className.search(
              /date-picker-unused|out-of-range|day-disabled|no-selection|not-selectable/
            ) != -1,
    ...
    this.setNewFocus = function() {
      ...
      if (td) {
        try {
          ...
          td.className =
            td.className.replace(/date-picker-hover/, "") +
            " date-picker-hover";
    ...
    this.updateTable = function(noCallback) {
      ...
      if (o.timerSet && !o.delayedUpdate) {
        if (o.monthInc) {
          ...
          o.date.setDate(
            Math.min(
              o.date.getDate() + o.dayInc,
              daysInMonth(
                o.date.getMonth() + o.monthInc,
                o.date.getFullYear() + o.yearInc
              )
            )
          );
      ...
      o.firstDateShown =
        !o.constrainSelection && o.fillGrid && 0 - firstColIndex < 1
          ? String(stubP) + (daySub + (0 - firstColIndex))
          : stub + "01";
      o.lastDateShown =
        !o.constrainSelection && o.fillGrid
          ? stubN + pad(41 - firstColIndex - dpm)
          : stub + String(dpm);
      ...
      dateSetD =
        o.dateSet != null
          ? o.dateSet.getFullYear() +
            pad(o.dateSet.getMonth() + 1) +
            pad(o.dateSet.getDate())
          : false;
      spn.className = "fd-screen-reader";
      ...
      for (var curr = 0; curr < 42; curr++) {
        ...
        if (
          (curr > firstColIndex && curr <= firstColIndex + dpm) ||
          o.fillGrid
        ) {
          currentStub = stub;
          weekDay = weekDayC;
          ...
          if (
            (o.rangeLow && +currentDate < +o.rangeLow) ||
            (o.rangeHigh && +currentDate > +o.rangeHigh)
          ) {
            ...
            if (selectable) {
              td.title = titleFormat
                ? printFormattedDate(
                    new Date(
                      +String(currentStub).substr(0, 4),
                      +String(currentStub).substr(4, 2) - 1,
                      +dt
                    ),
                    titleFormat,
                    true
              ...
              cName.push(
                "cd-" +
                  ...
                  currentStub +
                  " mmdd-" +
                  currentStub.substr(4, 2) +
                  pad(dt)
              );
            } else {
              td.title = titleFormat
                ? getTitleTranslation(13) +
                  " " +
                  printFormattedDate(
                    new Date(
                      +String(currentStub).substr(0, 4),
            ...
            if (o.disabledDays[weekDay] || dts[currentDate] == 0) {
            ...
            if (o.showWeeks) {
              cellAdded[row] = Math.min(
                cName[0] == "month-out" ? 3 : 1,
                cellAdded[row]
              );
            }
          }
        ...
        if (o.showWeeks && curr - row * 7 == 6) {
          ...
          o.wkThs[row].appendChild(
            document.createTextNode(
              cellAdded[row] == 4 && !o.fillGrid
                ? nbsp
                : getWeekNumber(cy, cm, curr - firstColIndex - 6)
            )
          );
          o.wkThs[row].className =
            "date-picker-week-header" +
            ["", "", " out-of-range", " month-out", ""][cellAdded[row]];
        }
      }
      ...
      span[0].appendChild(
        document.createTextNode(getMonthTranslation(cm, false) + nbsp)
      );
    ...
    this.destroy = function() {
      if (document.getElementById("fd-but-" + this.id)) {
        document
          .getElementById("fd-but-" + this.id)
          .parentNode.removeChild(document.getElementById("fd-but-" + this.id));
      }
    ...
    this.create = function() {
      ...
      if (!this.staticPos) {
        ...
        elem = document.getElementById(
          this.positioned ? this.positioned : this.id
        );
        if (!elem) {
          this.div = null;
          if (debug)
            throw this.positioned
              ? "Could not locate a datePickers associated parent element with an id:" +
                this.positioned
      ...
      for (var rows = 0; rows < 7; rows++) {
        ...
        for (var cols = 0; cols < colspanTotal; cols++) {
          if (rows === 0 || (this.showWeeks && cols === 0)) {
            ...
            setARIAProperty(
              ...
              this.id +
                "-col-" +
                cols +
                (this.showWeeks ? " " + this.id + "-row-" + rows : "")
            );
          ...
          row.appendChild(col);
          if (
            (this.showWeeks && cols > 0 && rows > 0) ||
            (!this.showWeeks && rows > 0)
          ) {
      ...
      for (var y = 0; y < colspanTotal; y++) {
        if (y == 0 && this.showWeeks) {
          this.ths[y].appendChild(
            document.createTextNode(getTitleTranslation(6))
          );
      ...
      if (
        this.rangeLow &&
        this.rangeHigh &&
        this.rangeHigh - this.rangeLow < 7
      ) {
        this.equaliseDates();
      }
    ...
    this.trackDrag = function(e) {
      ...
      o.div.style.left =
        Math.round(o.x + diffx) > 0 ? Math.round(o.x + diffx) + "px" : "0px";
      o.div.style.top =
        Math.round(o.y + diffy) > 0 ? Math.round(o.y + diffy) + "px" : "0px";
      /*@cc_on
                        @if(@_jscript_version <= 5.7)                         
      ...
      if (
        origEl.id.search(
          new RegExp(
            "^" +
              o.id +
              "(-prev-year-but|-prev-month-but|-next-month-but|-next-year-but)$"
          )
        ) != -1
      ) {
        ...
        if (!(o.currentYYYYMM == dateYYYYMM)) {
          if (
            (o.currentYYYYMM < dateYYYYMM &&
              (o.yearInc == -1 || o.monthInc == -1)) ||
            (o.currentYYYYMM > dateYYYYMM &&
              (o.yearInc == 1 || o.monthInc == 1))
          ) {
        ...
        var b = document.getElementsByTagName("body")[0];
        b.className =
          b.className.replace(/fd-drag-active/g, "") + " fd-drag-active";
    ...
    this.onclick = function(e) {
      ...
      while (el.parentNode) {
        ...
        if (el.tagName && el.tagName.toLowerCase() == "td") {
          if (
            el.className.search(/cd-([0-9]{8})/) == -1 ||
            el.className.search(
              /date-picker-unused|out-of-range|day-disabled|no-selection|not-selectable/
            ) != -1
          )
    ...
    this.show = function(autoFocus) {
      ...
      if (butt) {
        butt.className =
          butt.className.replace("dp-button-active", "") + " dp-button-active";
      }
    };
    this.hide = function() {
      if (
        !this.visible ||
        !this.created ||
        !document.getElementById("fd-" + this.id)
      )
        return;
    ...
    this.onfocus = function(e) {
      o.noFocus = false;
      o.div.className =
        o.div.className.replace("datepicker-focus", "") + " datepicker-focus";
      o.addOnFocusEvents();
    };
    ...
    this.onkeydown = function(e) {
      ...
      if (kc == 13) {
        ...
        if (
          !td ||
          td.className.search(/cd-([0-9]{8})/) == -1 ||
          td.className.search(/no-selection|out-of-range|day-disabled/) != -1
        ) {
          return stopEvent(e);
        }
        o.dateSet = new Date(o.date);
      ...
      switch (el.tagName.toLowerCase()) {
        ...
        case "th":
          ...
          if (el.className.search(/drag-enabled/) != -1) {
            ...
            statusText =
              txt.search(/^(\d+)$/) != -1
                ? getTitleTranslation(7, [
                    txt,
                    txt < 3 && o.date.getMonth() == 11
                      ? getWeeksInYear(o.date.getFullYear()) + 1
                      : getWeeksInYear(o.date.getFullYear())
                  ])
        ...
        case "span":
          ...
          if (el.className.search(/drag-enabled/) != -1) {
            ...
            statusText = getTitleTranslation(11, [
              getDayTranslation(day, false)
            ]);
            ...
            el.className.search(/today-but/) != -1 &&
            el.className.search(/disabled/) == -1
          ) {
    ...
    (function() {
      ...
      for (elemID in o.formElements) {
        ...
        if (
          elem &&
          elem.tagName &&
          elem.tagName.search(/select|input/i) != -1
        ) {
  ...
  datePicker.prototype.addButtonEvents = function(but) {
    ...
    if (!buttonTabIndex || this.bespokeTabIndex === false) {
      ...
      but.setAttribute(
        !/*@cc_on!@*/ false ? "tabIndex" : "tabindex",
        this.bespokeTabIndex
      );
  ...
  datePicker.prototype.addDisabledDates = function(dateObj) {
    ...
    for (startD in dateObj) {
      if (
        (String(startD).search(wcDateRegExp) != -1 && dateObj[startD] == 1) ||
        (String(startD).search(rangeRegExp) != -1 &&
          String(dateObj[startD]).search(rangeRegExp) != -1)
      ) {
  ...
  datePicker.prototype.getGenericDates = function(y, m, enabled) {
    ...
    for (dt in deDates) {
      ...
      if (dt2 == 1) {
        if (
          Number(dt1.substr(0, 6)) >=
            +String(this.firstDateShown).substr(0, 6) &&
      ...
      // Range
      if (
        +String(this.firstDateShown).substr(0, 6) >= Number(dt1.substr(0, 6)) &&
        ...
        rngLower =
          Number(dt1.substr(0, 6)) == +String(this.firstDateShown).substr(0, 6)
            ? dt1
            : lower;
        rngUpper =
          Number(dt2.substr(0, 6)) == +String(this.lastDateShown).substr(0, 6)
            ? dt2
            : upper;
  ...
  datePicker.prototype.outOfRange = function(tmpDate) {
    ...
    if (this.rangeLow && +dt < +this.rangeLow) {
      ...
      this.date = new Date(
        this.rangeLow.substr(0, 4),
        this.rangeLow.substr(4, 2) - 1,
        this.rangeLow.substr(6, 2),
        5,
        0,
        0
      );
    ...
    if (this.rangeHigh && +dt > +this.rangeHigh) {
      ...
      this.date = new Date(
        this.rangeHigh.substr(0, 4),
        this.rangeHigh.substr(4, 2) - 1,
        this.rangeHigh.substr(6, 2),
        5,
        0,
        0
      );
  ...
  datePicker.prototype.canDateBeSelected = function(tmpDate) {
    ...
    if (
      (this.rangeLow && +dt < +this.rangeLow) ||
      (this.rangeHigh && +dt > +this.rangeHigh) ||
      dd[dt] == 0 ||
      this.disabledDays[wd - 1]
    ) {
      return false;
    }
  ...
  datePicker.prototype.updateStatus = function(msg) {
    ...
    if (
      msg &&
      this.statusFormat.search(/-S|S-/) != -1 &&
      msg.search(/([0-9]{1,2})(st|nd|rd|th)/) != -1
    ) {
      msg = msg
        .replace(/([0-9]{1,2})(st|nd|rd|th)/, "$1<sup>$2</sup>")
        .split(/<sup>|<\/sup>/);
      ...
      this.statusBar.appendChild(
        document.createTextNode(msg ? msg : getTitleTranslation(9))
      );
    }
  };
  datePicker.prototype.setDateFromInput = function() {
    ...
    if (!dt || isNaN(dt)) {
      var newDate = new Date(
        ...
        !(m === false) ? m : new Date().getMonth(),
        1
      );
      this.date = this.cursorDate
        ? new Date(
            +this.cursorDate.substr(0, 4),
            +this.cursorDate.substr(4, 2) - 1,
            +this.cursorDate.substr(6, 2)
          )
        : new Date(
            newDate.getFullYear(),
            newDate.getMonth(),
            Math.min(
              +d || new Date().getDate(),
              daysInMonth(newDate.getMonth(), newDate.getFullYear())
            )
          );
  ...
  datePicker.prototype.disableDatePicker = function() {
    ...
    if (this.staticPos) {
      ...
      this.noFocus = true;
      this.div.className =
        this.div.className.replace(/dp-disabled/, "") + " dp-disabled";
      ...
      if (but) {
        but.className =
          but.className.replace(/dp-disabled/, "") + " dp-disabled";
  ...
  datePicker.prototype.disableTodayButton = function() {
    var today = new Date();
    this.butToday.className = this.butToday.className.replace(
      "fd-disabled",
      ""
    );
    if (
      this.outOfRange(today) ||
  ...
  datePicker.prototype.updateTableHeaders = function() {
    ...
    for (var col = colOffset; col < colspanTotal; col++) {
      ...
      if (col > colOffset) {
        ...
        this.ths[col].appendChild(
          document.createTextNode(getDayTranslation(d, true))
        );
      }

      this.ths[col].className = this.ths[col].className.replace(
        /date-picker-highlight/g,
        ""
      );
  ...
  datePicker.prototype.showHideButtons = function(tmpDate) {
    ...
    if (this.outOfRange(new Date(tdy - 1, tdm, daysInMonth(+tdm, tdy - 1)))) {
      ...
      this.butPrevYear.className = this.butPrevYear.className.replace(
        /fd-disabled/g,
        ""
      );
    }
    ...
    if (this.outOfRange(new Date(tdy, +tdm - 1, daysInMonth(+tdm - 1, tdy)))) {
      ...
      this.butPrevMonth.className = this.butPrevMonth.className.replace(
        /fd-disabled/g,
        ""
      );
    }
    ...
    if (this.outOfRange(new Date(tdy + 1, +tdm, 1))) {
      ...
      this.butNextYear.className = this.butNextYear.className.replace(
        /fd-disabled/g,
        ""
      );
    }
    ...
    if (this.outOfRange(new Date(tdy, +tdm + 1, 1))) {
      ...
      this.butNextMonth.className = this.butNextMonth.className.replace(
        /fd-disabled/g,
        ""
      );
    }
  ...
  var hideAll = function(exception) {
    ...
    for (dp in datePickers) {
      if (
        !datePickers[dp].created ||
        (exception && exception == datePickers[dp].id)
      )
        continue;
      datePickers[dp].hide();
    }
  ...
  var printFormattedDate = function(date, fmt, useImportedLocale) {
    ...
    var parts = fmt.split("-"),
      ...
      flags = {
        ...
        d: pad(d),
        D: useImportedLocale
          ? localeImport.dayAbbrs[D == 0 ? 6 : D - 1]
          : localeDefaults.dayAbbrs[D == 0 ? 6 : D - 1],
        l: useImportedLocale
          ? localeImport.fullDays[D == 0 ? 6 : D - 1]
          : localeDefaults.fullDays[D == 0 ? 6 : D - 1],
        j: d,
        N: D == 0 ? 7 : D,
        w: D,
        W: getWeekNumber(y, m, d),
        M: useImportedLocale
          ? localeImport.monthAbbrs[m]
          : localeDefaults.monthAbbrs[m],
        F: useImportedLocale
          ? localeImport.fullMonths[m]
  ...
  var parseDateString = function(str, fmt) {
    ...
    loopLabel: for (var pt = 0, part; (part = parts[pt]); pt++) {
      if (str.length == 0) {
        return false;
      }
      ...
      switch (part) {
        ...
        case "F": // A full textual representation of a month, such as January or March
          ...
          if (localeImport.imported) {
            l = l
              .concat(localeImport.fullMonths)
              .concat(localeImport.monthAbbrs);
          }
          for (var i = 0; i < l.length; i++) {
  ...
  var findLabelForElement = function(element) {
    ...
    if (
      element.parentNode &&
      element.parentNode.tagName.toLowerCase() == "label"
    )
      lebel = element.parentNode;
    else {
      ...
      for (var lbl = 0; lbl < labelList.length; lbl++) {
        ...
        if (
          (labelList[lbl]["htmlFor"] &&
            labelList[lbl]["htmlFor"] == element.id) ||
          labelList[lbl].getAttribute("for") == element.id
        ) {
          label = labelList[lbl];
          break;
        }
  ...
  var updateLanguage = function() {
    if (typeof window.fdLocale == "object") {
      localeImport = {
        ...
        firstDayOfWeek:
          "firstDayOfWeek" in fdLocale ? fdLocale.firstDayOfWeek : 0,
        imported: true
      };
    ...
    return !(
      ...
      !(
        (elem.tagName.toLowerCase() == "input" &&
          (elem.type == "text" || elem.type == "hidden")) ||
  ...
  var addDatePicker = function(options) {
    ...
    options.id =
      options.id && options.id in options.formElements ? options.id : "";
    options.formatMasks = {};
    ...
    for (var elemID in options.formElements) {
      ...
      if (!checkElem(elem)) {
        if (debug)
          throw "The element with and id of '" +
            elemID +
            "' is of the wrong type or does not exist within the DOM";
        return false;
      }
      ...
      if (
        options.dateFormats &&
        elemID in options.dateFormats &&
        options.dateFormats[elemID].length
      ) {
        newFormats = [];
      ...
      if (elem.tagName.toLowerCase() == "select") {
        ...
        if (tmpPartsFound[0] && tmpPartsFound[1] && tmpPartsFound[2]) {
          ...
          myMin += "" + "01";
          myMax +=
            "" + daysInMonth(+myMax.substr(4, 2) - 1, +myMax.substr(0, 4));
    ...
    var opts = {
      ...
      positioned:
        options.positioned && document.getElementById(options.positioned)
          ? options.positioned
          : "",
      // Ranges stipulated in YYYYMMDD format
      rangeLow:
        options.rangeLow && String(options.rangeLow).search(rangeRegExp) != -1
          ? options.rangeLow
          : "",
      rangeHigh:
        options.rangeHigh && String(options.rangeHigh).search(rangeRegExp) != -1
          ? options.rangeHigh
          : "",
      // Status bar format
      statusFormat:
        options.statusFormat &&
        String(options.statusFormat).search(validFmtRegExp) != -1
          ? options.statusFormat
          : "",
      ...
      dragDisabled:
        nodrag || !!options.staticPos ? true : !!options.dragDisabled,
      // Bespoke tabindex for this datePicker (or it's activation button)
      bespokeTabIndex:
        options.bespokeTabindex && typeof options.bespokeTabindex == "number"
          ? parseInt(options.bespokeTabindex, 10)
          : 0,
      // Bespoke titles
      ...
      finalOpacity:
        ...
        typeof options.finalOpacity == "number" &&
        (options.finalOpacity > 20 && options.finalOpacity <= 100)
          ? parseInt(+options.finalOpacity, 10)
          : !!options.staticPos ? 100 : finalOpacity,
      ...
      constrainSelection:
        "constrainSelection" in options ? !!options.constrainSelection : true,
      // The date to set the initial cursor to
      cursorDate:
        options.cursorDate &&
        String(options.cursorDate).search(rangeRegExp) != -1
          ? options.cursorDate
          : "",
      ...
      describedBy:
        options.describedBy && document.getElementById(options.describedBy)
          ? options.describedBy
      ...
      highlightDays:
        options.highlightDays &&
        options.highlightDays.length &&
        options.highlightDays.length == 7
          ? options.highlightDays
          : [0, 0, 0, 0, 0, 1, 1],
      // Days of the week to disable
      disabledDays:
        options.disabledDays &&
        options.disabledDays.length &&
        options.disabledDays.length == 7
          ? options.disabledDays
          : [0, 0, 0, 0, 0, 0, 0]
    };
    ...
    if (options.disabledDates) {
      ...
      for (startD in options.disabledDates) {
        if (
          (String(startD).search(wcDateRegExp) != -1 &&
            options.disabledDates[startD] == 1) ||
          (String(startD).search(rangeRegExp) != -1 &&
            String(options.disabledDates[startD]).search(rangeRegExp) != -1)
        ) {
      ...
      for (startD in options.enabledDates) {
        if (
          (String(startD).search(wcDateRegExp) != -1 &&
            options.enabledDates[startD] == 1) ||
          (String(startD).search(rangeRegExp) != -1 &&
            String(options.enabledDates[startD]).search(rangeRegExp) != -1)
        ) {
    ...
    datePickers[options.id].callback(
      "create",
      datePickers[options.id].createCbArgObj()
    );
  };
  ...
  return {
    ...
    setSelectedDate: function(inpID, yyyymmdd) {
      ...
      datePickers[inpID].setSelectedDate(yyyymmdd);
    },
    ...
    getSelectedDate: function(inpID) {
      return !inpID || !(inpID in datePickers)
        ? false
        : datePickers[inpID].returnSelectedDate();
    },
